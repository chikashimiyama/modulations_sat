...
static void zirkvbap(t_float g[3], long ls[3], t_zirkvbap *x)
{
  ...
  if(winner_set != x->x_previous_winner_set){
    t_atom floatValue;
    SETFLOAT(&floatValue, winner_set);
    outlet_anything(x->x_outlet4, gensym("selectedTriplet"), 1, &floatValue);
    x->x_previous_winner_set = winner_set;
  }
...
// read in loudspeaker matrices
{
 	...
 	while(counter-- > 0)
	{
   	...
   	t_atom atoms[3];
    SETFLOAT(&atoms[0], x->x_lsset[setpointer][0]);
    SETFLOAT(&atoms[1], x->x_lsset[setpointer][1]);
    if(x->x_dimension==3){
      SETFLOAT(&atoms[2], x->x_lsset[setpointer][2]);
    }
    outlet_anything(x->x_outlet4, gensym("tripletDefinition"), x->x_dimension, atoms);
